...
/** 按 URL 缓存解码后的 AudioBuffer，减少同曲切换难度时的重复下载与解码 */
const AUDIO_BUFFER_CACHE_MAX = 5;
const audioBufferCacheByUrl = new Map<string, AudioBuffer>();

function touchAudioBufferCache(url: string, buffer: AudioBuffer) {
  audioBufferCacheByUrl.delete(url);
  audioBufferCacheByUrl.set(url, buffer);
  while (audioBufferCacheByUrl.size > AUDIO_BUFFER_CACHE_MAX) {
    const k = audioBufferCacheByUrl.keys().next().value;
    if (k !== undefined) audioBufferCacheByUrl.delete(k);
  }
}

function getCachedAudioBuffer(url: string): AudioBuffer | undefined {
  const b = audioBufferCacheByUrl.get(url);
  if (b) touchAudioBufferCache(url, b);
  return b;
}
...
export function useMusicPlayer() {
  ...
  const lastSyncedMusicSessionRef = useRef<number | null>(null);

  const [isLoaded, setIsLoaded] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const musicSessionId = useGameStore((s) => s.musicSessionId);
  ...
  // 单独 effect：避免与「加载音频」effect 合并后依赖项个数变化（React 19 / HMR 会报错）
  useEffect(() => {
    if (!musicUrl) return;
    if (lastSyncedMusicSessionRef.current === musicSessionId) return;

    lastSyncedMusicSessionRef.current = musicSessionId;
    const state = audioStateRef.current;
    stopSource(true);
    state.audioBuffer = null;
    lastUrlRef.current = '';
    ...
    setIsLoading(false);
    setError(null);
  }, [musicSessionId, musicUrl, stopSource]);

  // 加载音频文件（在 pendingPlay 或音频缓存被清除时触发）
  useEffect(() => {
    const state = audioStateRef.current;

    // 需要加载：等待播放、URL 与当前已解码缓存不一致、或尚未有 buffer（含切歌后需换源）
    if (!musicUrl) return;

    const shouldLoad =
      pendingPlay ||
      lastUrlRef.current !== musicUrl ||
      !state.audioBuffer;
    ...
    const loadAudio = async () => {
      try {
        ...
        const cached = getCachedAudioBuffer(currentUrl);
        if (cached) {
        ...
        // 确保是当前 URL 的加载结果
        if (loadingUrlRef.current === currentUrl) {
          touchAudioBufferCache(currentUrl, audioBuffer);
